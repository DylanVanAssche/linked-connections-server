...
class EventsManager {
	...
	let isReferenced = false;
	    ...
	    this.previousEvents[agency][index]['referenced'] = true;
	}
    }
    ...
    async processEvents(timestamp) {
		...
		// Skip any referenced events that are already published
		...
		eventData.is_published = true;
            ...
            for(let e=0; e < this.previousEvents[agency].length; e++) {
                if(this.previousEvents[agency][e]['referenced']) {
		    this.previousEvents[agency][e]['referenced'] = false; // Reset for next round
                }
		else {
