...
class DatasetManager {
	...
	this._gtfsrt2lc_previous_connections = [];
    ...
    launchRTJob(index, dataset) {
        ...
        let rt_job = new cron.CronJob({
            ...
            onTick: async () => {
			    ...
			    if(jodata['@id'] in this._gtfsrt2lc_previous_connections) {
				let connection = this._gtfsrt2lc_previous_connections[jodata['@id']];
				/*if(connection['arrivalTime'] !== jodata['arrivalTime'] || connection['departureTime'] !== jodata['departureTime'] || connection['@type'] !== jodata['@type'])
				{
                                    logger.debug('Connection updated:' + jodata['@id'])
				}
				else {
				     logger.debug("Duplicate:" + jodata)
				     return;
				}*/
				logger.debug('Connection found!')
			    }
			    this._gtfsrt2lc_previous_connections.push(jodata['@id'])
