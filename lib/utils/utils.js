...
module.exports = new class Utils {
    ...
    async getRTIndex(arrays, lowLimit, highLimit, timeCriteria) {
        ...
        await Promise.all(arrays.map(async array => {
          ...
          // Sort
          /*array.sort((a, b) => {
              let a_date = new Date(a.split('"')[43]).getTime();
              let b_date = new Date(b.split('"')[43]).getTime();
              return a_date - b_date;
          });*/
          logger.debug("Number of items: " + array.length);
          array.filter((a) => {
            let obj = a.split('"');
            let a_date = new Date(obj[43]).getTime();
            let a_depDate = new Date(obj[19]);
            if(a_date <= timeCriteria && (a_depDate >= low && a_depDate < high)) {
              return true;
            }
            return false;
          });
          logger.debug("Filtered: " + array.length);
          for(let i in array) {
