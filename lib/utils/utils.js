...
module.exports = new class Utils {
	...
	logger.debug('RT INDEX=' + JSON.stringify(rt_index));
	logger.debug('RT REMOVE=' + JSON.stringify(rt_remove));
        ...
        for (let [connId, conn] of rt_index) {
            ...
            logger.debug('Processing RT connection:' + connId);
            logger.debug('Connection:' + JSON.stringify(conn, null, 4));
    ...
    async getRTIndex(arrays, static_index, lowLimit, highLimit, timeCriteria) {
		...
		let memento_date = new Date(obj[51]);

                // Discard invalid dates
                if (memento_date.toString() === 'Invalid Date') {
		    logger.warn('Memento data invalid, unable to process RT data for connection: ' + obj[3]);
                    continue;
                }
                ...
                if (memento_date <= timeCriteria) {
                    ...
                    if (depDate >= low && depDate < high) {
                        ...
                        if (map.has(connId)) {
                            ...
                            let sm = new Date(map.get(connId).split('"')[51]);
                            if (memento_date > sm) {
        ...
        for (let [id, memento] of possibleRemove) {
            //let sm = new Date(map.get(id).split('"')[43]);
            let sm = new Date(map.get(id).split('"')[51]);
            if (memento > sm) {
                map.delete(id);
            }
        }
