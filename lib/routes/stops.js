    ...
    createStopList(company) {
        return new Promise(async (resolve, reject) => {
            ...
            if (feeds.length > 0) {
                ...
                fs.createReadStream(this.storage + '/datasets/' + company + '/' + feed + '/stops.txt', { encoding: 'utf8', objectMode: true })
                    .pipe(csv.parse({ objectMode: true, headers: true }))
                    .on('data', data => {
                        skeleton['@graph'].push({
                            "@id": stops_uri_template.fill({ [stops_uri_template.varNames[0]]: data[stops_uri_template.varNames[0].split('.')[0]].trim() }),
