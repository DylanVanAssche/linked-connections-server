...
// Processes common request parameters
async function processRequest(req, res) {
    ...
    return [protocol, host, agency, departureTime]
}

router.get('/:agency/connections/static', async (req, res) => {
    let t0 = new Date();
    ...
    // Common processing for requests
    [protocol, host, agency, departureTime] = await processRequest(req, res);

    // Redirect to proper URL if final / is given before params
    if (req.url.indexOf('connections/static/') >= 0) {
        res.location('/' + agency + '/connections/static?departureTime=' + departureTime.toISOString());
        res.status(302).send();
        return;
    }
    ...
    try {
        ...
        if (departureTime.getTime() !== found_fragment) {
            res.location('/' + agency + '/connections/static?departureTime=' + ff.toISOString());
            res.status(302).send();
            return;
        }
...
router.get('/:agency/connections/live', async (req, res) => {
    ...
    try {
        ...
        //Redirect client to the apropriate fragment URL
        if (departureTime.getTime() !== found_fragment) {
            res.location('/' + agency + '/connections/live?departureTime=' + ff.toISOString());
            res.status(302).send();
            return;
        }
        ...
        if (rt_exists) {
            ...
            //jsonld_graph = await utils.aggregateRTData([], rt_data, rtfs_remove, lowLimit, highLimit, now); // TO DO, rewrite to support split
            jsonld_graph = [];
            t0 = new Date();
            let rt_index = await utils.getRTIndex(rt_data, lowLimit, highLimit, now); // 330 ms -> 280 ms by splitting the data
            logger.debug('getRTIndex() took ' + (new Date().getTime() - t0.getTime()) + ' ms');
            t0 = new Date();
            // Remove the for loop by bypassing the RTIndex
            for (let [connId, conn] of rt_index) {
                let rtd = null;
                let parsedRT = {};
                try {
                    rtd = JSON.parse(conn);
                    // Only show trains that have a delay
                    if(rtd['arrivalDelay'] === 0 && rtd['arrivalDelay'] === 0) {
                        continue;
                    }
                    parsedRT['@id'] = rtd['@id'];
                    parsedRT['@type'] = rtd['@type'];
                    parsedRT['arrivalDelay'] = rtd['arrivalDelay'];
                    parsedRT['departureDelay'] = rtd['departureDelay'];
                } catch (err) {
                    continue;
                }

                // If the connection is already present in the static fragment just add delay values and update departure and arrival times
                jsonld_graph.push(parsedRT);
            }
            ...
            t0 = new Date();
            await utils.addHydraMetada(params); // TO DO, should set the right URI for each resource
            logger.debug('Add Metadata took ' + (new Date().getTime() - t0.getTime()) + ' ms');
        }
        else {
            res.status(404).send();
